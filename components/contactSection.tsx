...
.cu-top-border {
  background: linear-gradient(90deg,
    transparent 0%,
    rgba(200,25,5,0.9) 20%,
    rgba(255,255,255,0.6) 50%,
    rgba(130,40,210,0.8) 80%,
    transparent 100%
  );
...
.cu-row::before {
  ...
  width: 2px;
  background: linear-gradient(180deg, transparent, rgba(200,25,5,0.9), transparent);
  opacity: 0;
  transition: opacity 0.3s ease;
  z-index: 2;
}
.cu-row:hover::before, .cu-row.is-active::before { opacity: 1; }
...
function CursorCard({ active, cursorX, cursorY }: { active: Contact | undefined; cursorX: ReturnType<typeof useSpring>; cursorY: ReturnType<typeof useSpring> }) {
        ...
        <AnimatePresence>
          {active && (
            <motion.div
              ...
              style={{ width: 210, height: 270, borderRadius: 10, overflow: "hidden", border: "1px solid rgba(200,25,5,0.35)", boxShadow: "0 0 0 1px rgba(130,40,210,0.12), 0 20px 50px rgba(0,0,0,0.85)", background: "#050000", position: "relative" }}
            >
              <div style={{ position: "absolute", inset: 0, zIndex: 0, backgroundImage: `url(${active.image})`, backgroundSize: "cover", backgroundPosition: "center", filter: "blur(18px) brightness(0.18) saturate(1.3)", transform: "scale(1.12)" }} />
              <img src={active.image} alt={active.name} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", zIndex: 1, display: "block", filter: "brightness(0.82) saturate(1.05)" }} />
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, zIndex: 4, background: `linear-gradient(90deg, transparent, ${T.red.glow} 40%, ${T.purple.glow} 70%, transparent)`, boxShadow: `0 0 8px ${T.red.glow}` }} />
              <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: 2, zIndex: 4, background: `linear-gradient(180deg, transparent, ${T.red.glow}, transparent)`, boxShadow: `0 0 6px ${T.red.glow}` }} />
              <div style={{ position: "absolute", inset: 0, zIndex: 2, background: "linear-gradient(180deg, transparent 35%, rgba(3,0,0,0.97) 100%)" }} />
...
const ContactRow = React.memo(function ContactRow({
  data, index, isActive, onEnter, onLeave, onToggle, isMobile, isAnyActive,
}: {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: isAnyActive && !isActive ? 0.18 : 1, x: 0 }}
      transition={{
        opacity: { duration: 0.35 },
        x: { duration: 0.55, delay: index * 0.06, ease: [0.22, 1, 0.36, 1] },
      }}
      ...
      <div style={{ position: "relative", zIndex: 1, padding: isMobile ? "18px 16px" : "22px 0", display: "flex", flexDirection: "column", gap: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            {!isMobile && (
              <span className="cu-role" style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>{data.position}</span>
            )}
            {isMobile ? (
        ...
        {isMobile && (
          <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)", marginTop: 4, paddingLeft: 28 }}>{data.position}</div>
        )}
      </div>
...
export default function ContactUsSection() {
  ...
  return (
    <div style={{ position: "relative", width: "100%", background: "radial-gradient(ellipse at 5% 10%, rgba(160,8,0,0.22) 0%, transparent 40%), radial-gradient(ellipse at 95% 90%, rgba(100,5,0,0.18) 0%, transparent 40%), #050000", padding: "clamp(40px, 5vw, 72px) clamp(20px, 6vw, 72px)", fontFamily: "'Rajdhani', sans-serif", cursor: "default" }}>
      ...
      {/* Shimmer top border */}
      <div className="cu-top-border" style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, zIndex: 10 }} />
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1100, margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <motion.h1
              ...
              style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(2rem, 5vw, 4.5rem)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "0.05em", textTransform: "uppercase", color: "#fff", margin: 0, textShadow: "0 0 28px rgba(200,25,5,0.5)" }}
            >
          ...
          {!isMobile && (
            <motion.div
              ...
              style={{ display: "flex", alignItems: "center", gap: 12, paddingBottom: 8 }}
            >
        ...
        <motion.div
          ...
          style={{ marginTop: 24, height: 1, background: `linear-gradient(90deg, ${T.red.glow}, ${T.purple.glow}, transparent)`, transformOrigin: "left" }}
        />
      </div>
